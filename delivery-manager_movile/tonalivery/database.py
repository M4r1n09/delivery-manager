...
class DatabaseManager:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                self.connection = psycopg2.connect(
                    connect_timeout=self.connection_timeout, **self.connection_params
                )
                self.connection.autocommit = True
                self.last_ping = datetime.now()
                print(f"✅ Conexión exitosa a la base de datos (intento {attempt + 1})")
                return True

            except Exception as e:
                print(
                    f"❌ Error conectando a la base de datos (intento {attempt + 1}/{self.max_retries}): {e}"
                )
    ...
    def _start_connection_monitor(self):
        """Inicia el monitoreo automático de la conexión"""
        ...
        def monitor():
            while True:
                ...
                if (
                    self.last_ping
                    and (datetime.now() - self.last_ping).seconds > self.ping_interval
                ):
                    if not self.ensure_connection():
                        print("⚠️ No se pudo mantener la conexión activa")

        monitor_thread = threading.Thread(target=monitor, daemon=True)
        monitor_thread.start()

    def execute_query(self, query, params=None, fetch=False):
        """Ejecuta una consulta con reconexión automática en caso de error"""
        if not self.ensure_connection():
